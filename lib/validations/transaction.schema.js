...
export const transactionSchema = z.object({
  ...
  (data) => {
    // If hasAdminFee is true, adminFeeAmount is required
    if (data.hasAdminFee) {
      return !!data.adminFeeAmount && data.adminFeeAmount.length > 0;
    }
    return true;
  },
  {
    message: 'Admin fee amount is required when admin fee is enabled',
    path: ['adminFeeAmount'],
  }
);
